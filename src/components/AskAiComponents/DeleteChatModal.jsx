...
import { deleteChat } from "../../store/slice/askAiSlice";
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (deleteChat.fulfilled.match(resultAction)) {
        ...
        console.error("Failed to delete chat:", resultAction.payload);
      }
    } catch (err) {
      ...
      >
        ...
        <div className="px-4 py-4">
          ...
          <div className="flex items-start gap-2 bg-red-50 border border-red-100 rounded-lg px-3 py-2.5">
            <Trash2 className="w-3 h-3 text-red-400 flex-shrink-0 mt-0.5" />
        ...
        <div className="flex items-center gap-2 px-4 pb-4">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 py-2 text-[12.5px] font-medium rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50 cursor-pointer"
          >
            {loading ? (
              <>
                <svg
