...
const AuditAiDashboard = () => {
  const [isFeedbackOpen, setIsFeedbackOpen] = useState(false);
  const navigate = useNavigate();
  const dispatch = useDispatch();
  // const { summary, countries, sectors, industries, loading, error } =
  //   useSelector((state) => state.databaseOverview);

  const data = useSelector((state) => state.auditAiDashboard.auditAiSummary);
  const loading = useSelector((state) => state.auditAiDashboard.loading);
  const error = useSelector((state) => state.auditAiDashboard.error);
  ...
  useEffect(() => {
    ...
    dispatch(fetchDates());
    document.title = "Audit Ai Dashboard";
  }, [dispatch]);

  const handleBegin = () => {
    navigate("/auditai-screening");
  };
  ...
  const statsData = [
    ...
    },
    {
      ...
      number:
        data?.type_breakdown?.find((item) => item.type === "409A")?.count || 0,
      label: "409A Questions",
      bgColor: "bg-green-50",
      iconBg: "bg-green-100",
    },
    {
      ...
      number:
        data?.type_breakdown?.find((item) => item.type === "PPA")?.count || 0,
      label: "PPA Questions",
    ...
    },
    {
      icon: allAssets.sectors,
      number:
        data?.type_breakdown?.find((item) => item.type === "Others (Note & Debt)")?.count || 0,
      label: "Others (Note & Debt) Questions",
      bgColor: "bg-purple-50",
      iconBg: "bg-purple-100",
    },
    {
      icon: allAssets.industries,
      number:
        data?.type_breakdown?.find((item) => item.type === "ASC 350-360")?.count || 0,
      label: "ASC 350-360 Questions",
      bgColor: "bg-green-50",
      iconBg: "bg-green-100",
    }
  ];
  ...
  const data409A =
    data?.category_breakdown
      ?.filter((item) => item.types.some((t) => t.type === "409A"))
      .map((item) => ({
  ...
  const dataPPA =
    data?.category_breakdown
      ?.filter((item) => item.types.some((t) => t.type === "PPA"))
      .map((item) => ({
        name: item.classification,
        count: item.total_count,
        color: "#F59E0B",
  ...
  const dataOthers =
    data?.category_breakdown
      ?.filter((item) => item.types.some((t) => t.type === "Others (Note & Debt)"))
      .map((item) => ({
        name: item.classification,
        count: item.total_count,
        color: "#6366F1",
  ...
  const dataASC350360 =
    data?.category_breakdown
      ?.filter((item) => item.types.some((t) => t.type === "ASC 350-360"))
      .map((item) => ({
        name: item.classification,
        count: item.total_count,
        color: "#6366F1",
  ...
  return (
    <div className="bg-gray-100 min-h-screen px-4 pt-6 pb-24  sm:px-32 lg:px-34">
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 mb-8 border border-gray-200">
        <div className="flex items-center justify-between mb-8">
          <h2 className="flex gap-2 text-xl sm:text-2xl font-bold text-gray-800 mb-6">
            Database Summary as of {date?.audit_date}
          </h2>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6">
        <FilterableList
          title="409A Categories"
          subtitle={`${
            data?.type_breakdown?.find((item) => item.type === "409A")?.count
          } Questions`}
          data={data409A}
        />
        <FilterableList
          title="PPA Categories"
          subtitle={`${
            data?.type_breakdown?.find((item) => item.type === "PPA")?.count
          } Questions`}
          data={dataPPA}
        />
        <FilterableList
          title="Others (Note & Debt) Categories"
          subtitle={`${
            data?.type_breakdown?.find((item) => item.type === "Others (Note & Debt)")?.count
          } Questions`}
          data={dataOthers}
          showSearch={true}
        ...
        />
        <FilterableList
          title="ASC 350-360 Categories"
          subtitle={`${data?.type_breakdown?.find((item) => item.type === "ASC 350-360")?.count
            } Questions`}
          data={dataASC350360}
          showSearch={true}
          showViewAllButton={true}
...
export default AuditAiDashboard;
